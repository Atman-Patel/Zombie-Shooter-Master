...
const NuclearBomb = {
  ...
  canvasElements: {
    img: '',
    imgSrc: './Guns/Nuke-container.png',
    imgCoords: {
      x: 460,
      y: 55,
      width: 129,
...
for (let i = 1; i <= 630; i++) {
  NuclearBomb.spread.push(i/100)
}
const setTimeLeft = (nuke) => {
  nuke.timeLeft = 20
  nuke.timer = setInterval(() => {
    nuke.timeLeft -= 1
  }, 1000)
  setTimeout(() => {
    clearInterval(nuke.timer)
  }, nuke.timeLeft*1000)
}
...
let keysArrow = ['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight']
let keysWASD = ['w', 'a', 's', 'd']
...
document.addEventListener('keydown', (e) => {
  if (keysArrow.includes(e.key)) {
    e.preventDefault();
    player.rotate(mouse)
  }
  if (keysWASD.includes(e.key)) {
    player.rotate(mouse)
  }
})
...
//gun related items
let gunSelector = [[Number(Pistol.num), Pistol], [Number(Smg.num), Smg], [Number(Ar.num), Ar], [Number(Sniper.num), Sniper], [Number(MachineGun.num), MachineGun], [Number(Shotgun.num), Shotgun], [Number(NuclearBomb.num), NuclearBomb]];
let gunElementSelector = [Pistol, Smg, Ar, Sniper, MachineGun, Shotgun, NuclearBomb];
...
canvas.addEventListener('mousedown', (e) => {
  ...
  timer = setInterval(() => {
    if (clicked && currentGun.autoFire) {
      if (currentGun.gun !== 'Sniper' && currentGun.gun !== 'Shotgun' && currentGun.gun !== 'Nuclear-Bomb') {
        bullets.push(
...
canvas.addEventListener("click", (e) => {
  ...
  if (!pause && currentGun.autoFire == false) {
    ...
    }else if(currentGun.gun == 'Nuclear-Bomb'){
      if(currentGun.timeLeft == 0){
        currentGun.spread.forEach((spr) => {
          bullets.push(
            new Bullet(player.vector.x, player.vector.y, player.angle + spr, ((currentGun.damage) / 100) * zombieHealth, currentGun.speed, currentGun.bulletRadius))
        })
        setTimeLeft(currentGun)
      }
      setTimeout(()=>{
        numBulletHit = bulHitZom
      },5000)
    }
